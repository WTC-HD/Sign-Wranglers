...
4. In the same Supabase project, create a second table named `events`, used for the site-wide stats shown at the top of the admin dashboard (feedback survey responses, polling-place lookup attempts, candidate directory visits):

   | Column | Type |
   |---|---|
   | `type` | text |

   Same as `signs`, `id` and `created_at` are added automatically. Enable RLS with no policies here too, for the same reason.

5. Sign up for [Resend](https://resend.com) and get an API key. Note: this project currently ships with email-sending commented out (see `app/api/requests/route.ts` and `app/api/pickup/route.ts`) until a real sending domain is verified — `onboarding@resend.dev`, Resend's default sandbox address, can only deliver to the account owner, not real users. Uncomment those blocks once you've verified your own domain with Resend.

6. Copy `.env.example` to a new file named `.env.local`, and fill in real values:
   ```bash
   ...
   - `GOOGLE_CIVIC_API_KEY` — from a Google Cloud project with the Civic Information API enabled (used for the polling-place lookup)

7. Customize for your own election:
...
Visit `/admin/login` and enter your `ADMIN_PASSWORD` to reach `/admin` — a dashboard showing site-wide stats (feedback survey results, polling-place lookup attempts, candidate directory visits) above a list of every sign request/pickup, with buttons to move each one through its status (`requested`/`pickup_requested` → `enroute` → `delivered`/`picked_up`).
...
3. Add the same environment variables from `.env.local` in Vercel's project settings during import.
